...
// Searches all parent or child pages and data sources that have been shared with
// an integration.
//
// Returns all pages or data sources, excluding duplicated linked data sources, that
// have titles that include the query param. If no query param is provided, then
// the response contains all pages or data sources that have been shared with the
// integration. The results adhere to any limitations related to an integration’s
// capabilities.

// To limit the request to search only pages or to search only data sources, use
// the filter param.
//
...
type SearchRequest struct {
	...
	// either only pages or only data sources. Possible value values are "page" or
	// "data_source". The only supported property value is "object".
